...
class llc {
  ...
  void controller(float reference, float rawAngle, float kneeAngle);
  int paused = 0;
  ...
  float kneeAngleComp = 0.0156; // motion: .02 // first:.0253
  float kneeAngle = 0.0;
  float rawAngle = 0.0;
  float errorDot = 0.0;
  float errorSum = 0.0;
  ...
  // Variables for bodged feed forward control
  float feedForwardCurrent = 3;
  long lastFeedForwardFlip = 0;
};
#endif
